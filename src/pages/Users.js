      ...
      <div className='flex-grow h-full flex-col'>
          {Accounts && Accounts.map((users,index) => (
            <li className='bg-purple-100 rounded-lg mr-3 ml-3 mb-3 items-center flex' key={users.uid}>
              ...
              <div className='w-3/5 text-bold text-3xl'>{users.Name}</div>
              <div className='w-1/5 text-bold text-3xl mt-[-12px] items-center flex'>
              <button onClick={() =>userClicked(index)} className="mt-3 shadow bg-orange-500 hover:bg-orange-300 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="button">
                Message
              </button></div>
            </li>
          ))}
